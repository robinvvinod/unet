...
def network(input_img, n_filters=16, dropout=0.5, batchnorm=True):
    ...
    c1 = inception_block(c0, n_filters=n_filters * 2, batchnorm=batchnorm, strides=2, recurrent=2, layers=((3,2),(5,1)))  # 256x256x256

    c2 = inception_block(c1, n_filters=n_filters * 4, batchnorm=batchnorm, strides=2, recurrent=2, layers=((3,2),(5,1)))  # 128x128x128

    c3 = inception_block(c2, n_filters=n_filters * 8, batchnorm=batchnorm, strides=2, recurrent=2, layers=((3,2),(5,1)))  # 64x64x64
    
    # bridge
    
    b0 = inception_block(c3, n_filters=n_filters * 16, batchnorm=batchnorm, strides=2, recurrent=2, layers=((3,2),(5,1)))  # 32x32x32
